...
def get_solver_path(solver_name):
    ...
    for p in [
        f"/home/adminuser/.conda/bin/{solver_name}",
        f"/opt/conda/bin/{solver_name}",
...
st.caption(
    "Mixed-Integer Non-Linear Programming Dashboard | Powered by MindtPy & COIN-OR"
)
...
# --- Main App Logic ---
tab_vis, tab_sens, tab_math = st.tabs(
    ["📊 ANALYSIS & VISUALIZATION", "📈 SENSITIVITY TRACE", "📝 MATH FORMULATION"]
)
...
if run_button:
    with st.spinner(f"Orchestrating {strategy} strategy..."):
        ...
        m_rel, s_rel, _ = (
            solve_minlp(c1_val, c2_val, c3_val, integer_constrained=False)
            if show_relaxation
            else (None, False, "")
        )
...
if st.session_state.res:
    ...
    with tab_vis:
        ...
        st.markdown(
            '<p class="metric-label">Optimal Objective Value</p>',
            unsafe_allow_html=True,
        )
        st.markdown(
            f'<p class="metric-value">{pyo.value(m.obj):.4f}</p>',
            unsafe_allow_html=True,
        )
        ...
        with col_m2:
            if m_rel:
                st.info(
                    f"💡 Relaxation GAP: **{((pyo.value(m_rel.obj) - pyo.value(m.obj)) / pyo.value(m_rel.obj) * 100):.2f}%**"
                )
        ...
        st.info(
            "💡 **Visualization Tip**: The **shaded dark areas** are infeasible (constraint-breaking). The optimal solution always sits in the bright feasible region."
        )
        ...
        fig.update_layout(
            ...
            scene=dict(
                xaxis=dict(
                    gridcolor="rgba(255,255,255,0.1)", backgroundcolor="rgba(0,0,0,0)"
                ),
                yaxis=dict(
                    gridcolor="rgba(255,255,255,0.1)", backgroundcolor="rgba(0,0,0,0)"
                ),
        ...
        st.caption(
            "✨ Tip: Rotate the 3D plot to see how the optimal point sits on the surface gradient."
        )
    ...
    with tab_sens:
        ...
        st.write(
            "Visualizing how varying $c_1$ (Constraint 1 constant) impacts the final outcome."
        )
        ...
        with st.status("Walking through parameter space...") as status:
            for test_c in c1_range:
                ...
                model_r, ok_r, _ = solve_minlp(
                    test_c, c2_val, c3_val, strategy="OA", integer_constrained=False
                )
                if ok_r:
                    sens_data.append(
                        {
        ...
        fig_sens.add_vline(
            x=c1_val, line_dash="dash", line_color="red", annotation_text="Current C1"
        )
    ...
    with tab_math:
        ...
        st.success(
            "This model is a classic Mixed-Integer Non-Linear Programming (MINLP) problem because it contains both integer variables and non-linear terms ($yx$)."
        )
else:
    with tab_vis:
        st.info(
            "Adjust the parameters in the sidebar and click **RUN OPTIMIZATION** to begin."
        )
